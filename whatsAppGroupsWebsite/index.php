    ...
    <header>
        ...
        <ul>
            ...
            if (isset($_COOKIE["username"])) {
                ...
                echo '<li><a class="ab" href="Settings.php">الأعدادات</a></li>';
...
<div class="login-box " style=" background-color: #161616;  top: 200px; height: auto; width: 70%">
    ...
    <table class="table table-striped" style="top:0px ; color: #ffffff; text-align: center">
        ...
        if (isset($_POST['searchTerm'])) {
            ...
            $get_gruopName = "SELECT id,course_id,course_number,group_name,courselink,comments.groupID,nParticipants,AVG(rating) AS ra FROM usergroup,comments WHERE id= comments.groupID and (group_name like '%$searchTerm%' OR course_id like '%$searchTerm%') group By comments.groupID order by ra DESC"  ;
            $res_group_name = mysqli_query($conn, $get_gruopName);
        } else {
            $get_gruopName = "SELECT id,course_id,course_number,group_name,courselink,comments.groupID,nParticipants,AVG(rating) AS ra FROM usergroup,comments WHERE id= comments.groupID  group by comments.groupID order by ra DESC  ";
        ...
        if (mysqli_num_rows($res_group_name) > 0) {
            ...
            while ($row = $res_group_name->fetch_assoc()  ) {
                ...
                echo '<td><a href="https://chat.whatsapp.com/' . $row["courselink"] . '" class="aButton" style="pointer-events: all">انضمام</a></td></tr>';
        ...
        if (isset($_POST['searchTerm'])) {
            ...
            $group_name = null;
            $group_name == stripcslashes(strtolower($_POST['searchTerm']));
            $get_gruopName = "SELECT id,course_id,course_number,group_name,courselink,nParticipants,0 AS ra FROM usergroup WHERE id not in(select comments.groupID from usergroup,comments where id=comments.groupID ) and (group_name like '%$searchTerm%' OR course_id like '%$searchTerm%') "  ;
            $res_group_name = mysqli_query($conn, $get_gruopName);

        } else {
            $get_gruopName = "SELECT id,course_id,course_number,group_name,courselink,nParticipants,0 AS ra FROM usergroup WHERE id not in(select comments.groupID from usergroup,comments where id=comments.groupID )  ";
